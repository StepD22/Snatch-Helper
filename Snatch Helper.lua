...
local S = {
    ...
    isActiveDemouteCommand = false,
    selectedDemoutePlayerId = 0,
    selectedDemoutePlayerName = "",
    isActiveDismissCommand = false,
...
local MembersOverlayWindow = imgui.new.bool(false)
...
local function forceWindowPadding(x, y)
...
local function anim(id, target, speed)
    speed = speed or 0.10
    if anims[id] == nil then anims[id] = target end
...
local function animSpring(id, target, stiffness, damping)
    ...
    local key_v = id .. "_vel"
    local key_p = id .. "_pos"
    if anims[key_p] == nil then anims[key_p] = target; anims[key_v] = 0 end
...
local function cleanupAnims()
    ...
    if count > C.ANIM_MAX_ENTRIES then
        local toRemove = {}
        for k, v in pairs(anims) do
            if type(k) == "string" and k:sub(-4) == "_vel" then
                if math.abs(v) < 0.001 then table.insert(toRemove, k) end
            end
        end
        for _, k in ipairs(toRemove) do anims[k] = nil end
        count = 0
        for _ in pairs(anims) do count = count + 1 end
        if count > C.ANIM_MAX_ENTRIES * 1.5 then anims = {} end
    end
end
...
local function generateId()
    return string.format("%x%x%x", os.time(), math.random(0, 0xFFFF), math.random(0, 0xFF))
end

local function deepCopy(t)
    if type(t) ~= "table" then return t end
    local c = {}; for k, v in pairs(t) do c[k] = deepCopy(v) end
...
local ease = {
    outCubic = function(t) return 1 - math.pow(1 - clamp(t,0,1), 3) end,
    inOutCubic = function(t) t = clamp(t,0,1); if t < 0.5 then return 4*t*t*t end; return 1 - math.pow(-2*t+2,3)/2 end,
    outBack = function(t) t = clamp(t,0,1); local c1 = 1.70158; local c3 = c1+1; return 1+c3*math.pow(t-1,3)+c1*math.pow(t-1,2) end,
    outQuart = function(t) return 1 - math.pow(1 - clamp(t,0,1), 4) end,
}
...
local notifyIcons = {
    [C.NOTIFY_INFO]="i", [C.NOTIFY_SUCCESS]="+", [C.NOTIFY_WARNING]="!",
    [C.NOTIFY_ERROR]="x", [C.NOTIFY_SYSTEM]="*",
}
...
local function addNotification(text, ntype, duration)
    ...
    table.insert(notifications, { text = text, ntype = ntype, time = os.clock(), duration = duration })
    if #notifications > C.MAX_NOTIFICATIONS then table.remove(notifications, 1) end
end
...
local function drawSparkline(dl, x, y, w, h, data, color, filled)
    ...
    local maxV = 0
    for _, v in ipairs(data) do if v > maxV then maxV = v end end
    if maxV == 0 then maxV = 1 end
    local step = w / (#data - 1)
    local points = {}
    for i, v in ipairs(data) do
        local px = x + (i-1) * step
        local py = y + h - (v / maxV) * h
    ...
    if filled then
        for i = 1, #points - 1 do
            ...
            dl:AddRectFilledMultiColor(
                imgui.ImVec2(p1[1], p1[2]), imgui.ImVec2(p2[1], y+h),
                ImVec4toU32(imgui.ImVec4(color.x,color.y,color.z,0.15)),
                ImVec4toU32(imgui.ImVec4(color.x,color.y,color.z,0.15)),
                ImVec4toU32(imgui.ImVec4(color.x,color.y,color.z,0.02)),
                ImVec4toU32(imgui.ImVec4(color.x,color.y,color.z,0.02))
            )
        end
    end
    for i = 1, #points - 1 do
        dl:AddLine(imgui.ImVec2(points[i][1], points[i][2]),
            imgui.ImVec2(points[i+1][1], points[i+1][2]), ImVec4toU32(color), 2)
    end
    if #points > 0 then
        local lp = points[#points]
        dl:AddCircleFilled(imgui.ImVec2(lp[1],lp[2]), 3, ImVec4toU32(color), 12)
        drawGlow(dl, lp[1], lp[2], 3, color, 0.4)
    end
end
...
local function gradientNick(text, hex1, hex2)
    ...
    for i = 1, len do
        ...
        result = result .. string.format("{%02X%02X%02X}%s", r, g, b, text:sub(i, i))
    end
    return result
end
...
local settings = {
    ...
    openKey = 0x72, binderKey = 0x73, compactMode = false, showParticles = true,
...
local function loadSettings()
    ...
    if ok and type(data) == "table" then
        for k, v in pairs(data) do
            if settings[k] ~= nil and type(settings[k]) == type(v) then settings[k] = v end
        end
    end
end
...
local colorThemes = {
    ...
    obsidian = {
        name = "Obsidian", icon = "$",
        sidebar        = imgui.ImVec4(0.025, 0.025, 0.028, 1),
        sidebar_active = imgui.ImVec4(0.055, 0.055, 0.062, 1),
        bg             = imgui.ImVec4(0.035, 0.035, 0.040, 0.99),
        header         = imgui.ImVec4(0.028, 0.028, 0.032, 1),
        card           = imgui.ImVec4(0.058, 0.058, 0.065, 0.92),
        card_hover     = imgui.ImVec4(0.078, 0.078, 0.088, 0.95),
        input          = imgui.ImVec4(0.040, 0.040, 0.048, 0.90),
        accent         = imgui.ImVec4(0.78, 0.80, 0.85, 1),
        accent2        = imgui.ImVec4(0.90, 0.92, 0.95, 1),
        accent_dim     = imgui.ImVec4(0.55, 0.58, 0.62, 1),
        text           = imgui.ImVec4(0.88, 0.88, 0.90, 1),
        text2          = imgui.ImVec4(0.55, 0.55, 0.58, 1),
        text3          = imgui.ImVec4(0.35, 0.35, 0.38, 1),
        green          = imgui.ImVec4(0.30, 0.88, 0.55, 1),
        yellow         = imgui.ImVec4(1.00, 0.85, 0.30, 1),
        red            = imgui.ImVec4(1.00, 0.35, 0.35, 1),
        blue           = imgui.ImVec4(0.40, 0.68, 1.00, 1),
        grad1          = imgui.ImVec4(0.60, 0.62, 0.68, 1),
        grad2          = imgui.ImVec4(0.35, 0.38, 0.45, 1),
        glass          = imgui.ImVec4(0.04, 0.04, 0.05, 0.70),
    },
    aurora = {
        name = "Aurora", icon = "@",
    ...
    crimson = {
        name = "Crimson", icon = "&",
        sidebar        = imgui.ImVec4(0.045, 0.018, 0.022, 1),
        sidebar_active = imgui.ImVec4(0.092, 0.032, 0.042, 1),
        bg             = imgui.ImVec4(0.058, 0.022, 0.028, 0.99),
        header         = imgui.ImVec4(0.048, 0.020, 0.025, 1),
        card           = imgui.ImVec4(0.085, 0.035, 0.045, 0.92),
        card_hover     = imgui.ImVec4(0.115, 0.048, 0.058, 0.95),
        input          = imgui.ImVec4(0.058, 0.025, 0.032, 0.90),
        accent         = imgui.ImVec4(0.85, 0.12, 0.22, 1),
        accent2        = imgui.ImVec4(1.00, 0.30, 0.40, 1),
        accent_dim     = imgui.ImVec4(0.62, 0.08, 0.15, 1),
        text           = imgui.ImVec4(1.00, 0.94, 0.95, 1),
        text2          = imgui.ImVec4(0.70, 0.50, 0.54, 1),
        text3          = imgui.ImVec4(0.48, 0.32, 0.36, 1),
        green          = imgui.ImVec4(0.30, 0.88, 0.50, 1),
        yellow         = imgui.ImVec4(1.00, 0.82, 0.25, 1),
        red            = imgui.ImVec4(1.00, 0.20, 0.30, 1),
        blue           = imgui.ImVec4(0.40, 0.65, 1.00, 1),
        grad1          = imgui.ImVec4(0.80, 0.08, 0.18, 1),
        grad2          = imgui.ImVec4(0.45, 0.00, 0.12, 1),
        glass          = imgui.ImVec4(0.08, 0.03, 0.04, 0.70),
    },
    sakura = {
        name = "Sakura", icon = "^",
...
local function afCommand(arg)
    ...
    if id then
        ...
        if nick == autoFind.lastNick then addNotification("AutoFind: поиск " .. autoFind.lastNick .. " завершён", C.NOTIFY_INFO); afReset(); return end
...
local function checkModifier(m)
    ...
    if m==1 then return isKeyDown(0x11) end
    if m==2 then return isKeyDown(0x10) end
    if m==3 then return isKeyDown(0x12) end
    return false
end

local function checkKeyCombo(k, m)
    if k==0 or not wasKeyPressed(k) then return false end
...
local function initVariables()
    simpleVariables = {
        my_id        = { desc = "Ваш ID",        cat = "Игрок",   func = function() local _,id = sampGetPlayerIdByCharHandle(playerPed); return id or -1 end },
        my_nick      = { desc = "Ваш ник",       cat = "Игрок",   func = function() local _,id = sampGetPlayerIdByCharHandle(playerPed); return id and sampGetPlayerNickname(id) or "Unknown" end },
        my_rpnick    = { desc = "РП-ник",         cat = "Игрок",   func = function() local _,id = sampGetPlayerIdByCharHandle(playerPed); return id and sampGetPlayerNickname(id):gsub("_"," ") or "Unknown" end },
        my_name      = { desc = "Имя",            cat = "Игрок",   func = function() local _,id = sampGetPlayerIdByCharHandle(playerPed); if not id then return "?" end; local n=sampGetPlayerNickname(id); return n:match("^(.-)_") or n end },
        my_surname   = { desc = "Фамилия",        cat = "Игрок",   func = function() local _,id = sampGetPlayerIdByCharHandle(playerPed); if not id then return "?" end; local n=sampGetPlayerNickname(id); return n:match("_(.+)$") or n end },
        my_hp        = { desc = "HP",             cat = "Статы",   func = function() local _,id = sampGetPlayerIdByCharHandle(playerPed); return id and (sampGetPlayerHealth(id) or 0) or 0 end },
        my_armor     = { desc = "Броня",          cat = "Статы",   func = function() local _,id = sampGetPlayerIdByCharHandle(playerPed); return id and (sampGetPlayerArmor(id) or 0) or 0 end },
        my_ping      = { desc = "Пинг",           cat = "Статы",   func = function() local _,id = sampGetPlayerIdByCharHandle(playerPed); return id and (sampGetPlayerPing(id) or 0) or 0 end },
        my_score     = { desc = "Уровень",        cat = "Статы",   func = function() local _,id = sampGetPlayerIdByCharHandle(playerPed); return id and (sampGetPlayerScore(id) or 0) or 0 end },
        my_weapon    = { desc = "ID оружия",      cat = "Статы",   func = function() return getCurrentCharWeapon(playerPed) or 0 end },
        my_x         = { desc = "X",              cat = "Мир",     func = function() local x,_,_ = getCharCoordinates(playerPed); return math.floor(x) end },
        my_y         = { desc = "Y",              cat = "Мир",     func = function() local _,y,_ = getCharCoordinates(playerPed); return math.floor(y) end },
        my_z         = { desc = "Z",              cat = "Мир",     func = function() local _,_,z = getCharCoordinates(playerPed); return math.floor(z) end },
        ...
        day          = { desc = "День недели",    cat = "Время",   func = function() local days = {"Вс","Пн","Вт","Ср","Чт","Пт","Сб"}; return days[tonumber(os.date("%w"))+1] or "?" end },
...
local function resolveVariable(vn, cache)
    cache = cache or {}
    if cache[vn] then return cache[vn] end
    local entry = simpleVariables[vn]
    if entry and entry.func then
        local ok, val = pcall(entry.func)
        if ok and val ~= nil then cache[vn] = tostring(val); return cache[vn] end
    end
    return nil
end
...
local function saveActivityLog()
    if not ensureDirectory(configDirectory) then return end
    local f = io.open(U.activityLogFile or (configDirectory .. "/activity_log.json"), 'w'); if not f then return end
    local sv = {}
...
local function loadActivityLog()
    if not doesFileExistSafe(U.activityLogFile or (configDirectory .. "/activity_log.json")) then return end
    local f = io.open(U.activityLogFile or (configDirectory .. "/activity_log.json"), 'r')
    local c = f:read('*a'); f:close()
    if not c or #c < 2 then return end
    local ok, data = pcall(decodeJson, c)
    if ok and type(data) == "table" then
...
local function joinSteps(steps) return steps and table.concat(steps, "\n") or "" end
-- =========================================================================
...
local function discardAndSelect(idx)
    ...
    if idx then
        ...
        ffi.copy(B.editCmdBuf, b.cmd and ("/" .. b.cmd) or "")
...
local function loadBinds()
    binds = {}
    if not doesFileExistSafe(U.bindsConfigFile or (configDirectory .. "/binds.json")) then return end
    local f = io.open(U.bindsConfigFile or (configDirectory .. "/binds.json"), 'r')
    local c = f:read('*a'); f:close()
    if not c or #c < 2 then return end
    local ok, data = pcall(decodeJson, c)
    if ok and type(data) == "table" then
...
local function saveBinds()
    if not ensureDirectory(configDirectory) then return end
    local f = io.open(U.bindsConfigFile or (configDirectory .. "/binds.json"), 'w'); if not f then return end
    local sv = {}
...
local function extractCustomVars(steps)
    ...
    for _, s in ipairs(steps) do
        for v in s:gmatch("{([^}]+)}") do
            if not seen[v] and not simpleVariables[v] then seen[v] = true; table.insert(vars, v) end
        end
    end; return vars
end
...
local function processStep(step, subs, cache)
    local r = step
    if subs then for v, val in pairs(subs) do r = r:gsub("{" .. v .. "}", val) end end
    for vn, _ in pairs(simpleVariables) do
        local p = "{" .. vn .. "}"
        if r:find(p, 1, true) then
            local val = resolveVariable(vn, cache)
            if val then r = r:gsub(p, val:gsub("%%", "%%%%")) end
        end
    end
    return r
end
...
performBind = function(bind, args)
    ...
    addActivity(">> " .. S.currentBindName, "bind")
    updateHourlyStats()
    local cache = {}
    local success = true
    for si, step in ipairs(bind.steps) do
        ...
        if S.stopCurrentBind then
            addActivity("STOP " .. S.currentBindName, "system")
        ...
        elseif step:match("^%[wait_random:(%d+),(%d+)%]$") then
            local mn, mx = step:match("^%[wait_random:(%d+),(%d+)%]$")
            wait(math.random(tonumber(mn), tonumber(mx)))
        elseif step:match("^%[if:hp(%D)(%d+)%](.+)$") then
            local op, val, cmd = step:match("^%[if:hp(%D)(%d+)%](.+)$")
            ...
            if cond then safeSendChat(processStep(trim(cmd), subs, cache)); wait(delay) end
        elseif step:match("^%[repeat:(%d+)%](.+)$") then
            local count, cmd = step:match("^%[repeat:(%d+)%](.+)$")
            count = tonumber(count) or 1
            for _ = 1, math.min(count, 10) do
                if S.stopCurrentBind then break end
                safeSendChat(processStep(trim(cmd), subs, cache)); wait(delay)
            end
        else
            safeSendChat(processStep(step, subs, cache)); wait(delay)
        end
    end
...
local function secTitle(text)
    local t = T()
    imgui.Dummy(imgui.ImVec2(0, 5))
    local sp = imgui.GetCursorScreenPos(); local dl = imgui.GetWindowDrawList()
    dl:AddCircleFilled(imgui.ImVec2(sp.x + 4, sp.y + 8), 3.5, ImVec4toU32(t.accent, 0.75), 14)
    drawGlow(dl, sp.x + 4, sp.y + 8, 4, t.accent, 0.25)
    imgui.Dummy(imgui.ImVec2(16, 0)); imgui.SameLine()
    if U.fontSemibold then imgui.PushFont(U.fontSemibold) end
    imgui.PushStyleColor(imgui.Col.Text, t.accent)
    imgui.Text(u8(text:upper())); imgui.PopStyleColor()
    if U.fontSemibold then imgui.PopFont() end
    ...
    function(self)
        ...
        if S.waitingForKey then
            ...
            if btn(" Применить ", 100, 32, "success") then
                ...
                elseif S.keyBindContext == "stop" then settings.stopKey = S.tempKey; settings.stopKeyMod = S.tempMod; saveSettings() end
...
11) Задаём вопросы
12) Оповещаем о завершении допроса и выключаем камеру
13) Выводим допрашиваемого с мешком на голове из офиса]] },
...
imgui.OnFrame(
    ...
    function(self)
        ...
        local t = T()
        local oW = 300
        local posY = settings.overlayPosition == 0 and 12 or (sizeY - 62)
        imgui.SetNextWindowPos(imgui.ImVec2(sizeX/2, posY), imgui.Cond.Always, imgui.ImVec2(0.5,0))
        imgui.SetNextWindowSize(imgui.ImVec2(oW, 56), imgui.Cond.Always)
        imgui.PushStyleVarFloat(imgui.StyleVar.WindowRounding, 20)
        imgui.PushStyleVarFloat(imgui.StyleVar.WindowBorderSize, 0)
        pushWindowPadding(18,10)
        imgui.PushStyleColor(imgui.Col.WindowBg, imgui.ImVec4(t.sidebar.x,t.sidebar.y,t.sidebar.z,0.94))
        ...
        local dl = imgui.GetWindowDrawList()
        local wp = imgui.GetWindowPos(); local ws = imgui.GetWindowSize()
        drawShadow(dl, wp.x, wp.y, wp.x+ws.x, wp.y+ws.y, 20, 6, 0.6)
        pulsingDot(dl, wp.x+18, wp.y+20, 4, t.yellow, 5)
        imgui.Dummy(imgui.ImVec2(26,0)); imgui.SameLine()
        imgui.PushStyleColor(imgui.Col.Text, t.yellow)
        local nm = #S.currentBindName > 20 and S.currentBindName:sub(1,18)..".." or S.currentBindName
        imgui.Text(u8(nm)); imgui.PopStyleColor()
        imgui.SameLine(oW - 68)
        imgui.PushStyleColor(imgui.Col.Text, t.text3)
        imgui.Text(u8(safeFormat("%d/%d", S.currentBindProgress, S.currentBindTotal))); imgui.PopStyleColor()
        local prog = clamp(S.currentBindProgress / math.max(S.currentBindTotal,1), 0, 1)
        local barY = wp.y + 43
        local barProg = anim("overlay_prog", prog, 0.08)
        local barInset = 18
        drawGrad(dl, wp.x+barInset, barY, (oW-barInset*2)*barProg, 4, t.yellow,
            imgui.ImVec4(t.yellow.x,t.yellow.y,t.yellow.z,0.12))
...
imgui.OnFrame(
    ...
    function(self)
        ...
        for i, n in ipairs(notifications) do
            ...
            if U.fontSemibold then imgui.PushFont(U.fontSemibold) end
imgui.PushStyleColor(imgui.Col.Text, imgui.ImVec4(col.x,col.y,col.z,alpha))
imgui.Text(u8(" " .. (notifyIcons[n.ntype] or "i") .. " ")); imgui.PopStyleColor()
if U.fontSemibold then imgui.PopFont() end
            imgui.SameLine()
...
local function drawDashBanner(dl, contentW, t, ps, now)
    ...
    local avatarR = 30
    if avatarTexture ~= nil then
        local imgR = avatarR + 3
        dl:AddImageRounded(avatarTexture,
...
local function drawDashMetrics(dl, contentW, t, ps)
    ...
    local cardH = 88
    local metrics = {
        { icon = "@", val = ps.online, label = "Онлайн", color = t.blue, spark = nil },
        { icon = "~", val = ps.fps, label = "FPS",
            color = ps.fps > 30 and t.green or (ps.fps > 15 and t.yellow or t.red), spark = dashData.fpsHistory },
        { icon = "!", val = ps.ping, label = "Ping",
            color = ps.ping < 80 and t.green or (ps.ping < 150 and t.yellow or t.red), spark = dashData.pingHistory },
    }
    for mi, m in ipairs(metrics) do
        ...
        if m.spark and #m.spark >= 2 then
            drawSparkline(cdl, cwp.x + cardW - 65, cwp.y + 12, 52, 30, m.spark,
                imgui.ImVec4(m.color.x, m.color.y, m.color.z, 0.18 + mhov * 0.12), true)
        end
...
local function drawDashChart(contentW, t)
    secTitle("График активности")
    cardBegin("##activity_chart", imgui.ImVec2(contentW, 220))
    imgui.PushStyleColor(imgui.Col.Text, t.text2)
    imgui.Text(u8("Активность по часам")); imgui.PopStyleColor()
    imgui.Spacing()
    local acDl = imgui.GetWindowDrawList()
    local acPos = imgui.GetCursorScreenPos()
    local acW = imgui.GetContentRegionAvail().x - 8; local acH = 130
    local hourData = {}; local maxH2 = 1
    for h = 0, 23 do hourData[h] = hourlyStats[h] or 0; if hourData[h] > maxH2 then maxH2 = hourData[h] end end
    local barW3 = math.max(3, (acW / 24) - 2)
    for h = 0, 23 do
        local bx2 = acPos.x + h * (barW3 + 2)
        local bh = (hourData[h] / maxH2) * (acH - 20)
        local isCurrent = h == tonumber(os.date("%H"))
        local barCol = isCurrent and t.accent or imgui.ImVec4(t.accent.x, t.accent.y, t.accent.z, 0.25)
        acDl:AddRectFilled(imgui.ImVec2(bx2, acPos.y + acH - 20 - bh),
            imgui.ImVec2(bx2 + barW3, acPos.y + acH - 20), ImVec4toU32(barCol), 3)
        if isCurrent then drawGlow(acDl, bx2 + barW3/2, acPos.y + acH - 20 - bh, 3, t.accent, 0.35) end
        if h % 4 == 0 then
            acDl:AddText(imgui.ImVec2(bx2, acPos.y + acH - 14), ImVec4toU32(t.text3, 0.4), u8(safeFormat("%02d", h)))
        end
    end
    imgui.Dummy(imgui.ImVec2(acW, acH)); cardEnd()
    imgui.Spacing(); thinSep()
end
...
local function drawDashboard()
    ...
    if now - S.lastFpsUpdate > 0.5 then
        S.lastFpsUpdate = now
        table.insert(dashData.fpsHistory, ps.fps); table.insert(dashData.pingHistory, ps.ping)
        while #dashData.fpsHistory > 30 do table.remove(dashData.fpsHistory, 1) end
        while #dashData.pingHistory > 30 do table.remove(dashData.pingHistory, 1) end
    end
...
do
    ...
    for _, ci in ipairs(row1) do
        ...
        imgui.PushStyleColor(imgui.Col.ButtonHovered,
            imgui.ImVec4(catCol.x, catCol.y, catCol.z, 0.25))
        imgui.PushStyleColor(imgui.Col.ButtonActive,
            imgui.ImVec4(catCol.x, catCol.y, catCol.z, 0.4))
        imgui.PushStyleColor(imgui.Col.Text,
            isActive and imgui.ImVec4(1, 1, 1, 1) or lerpColor(t.text3, t.text2, hov))
        ...
        if imgui.Button(u8(cs[ci + 1]) .. "##catr" .. ci, imgui.ImVec2(row1W, tabH)) then
            U.filterCategory[0] = ci
        end
        
        -- Нижний индикатор
        if isActive then
            local mi = imgui.GetItemRectMin(); local ma = imgui.GetItemRectMax()
            local dl2 = imgui.GetWindowDrawList()
            dl2:AddRectFilled(
                imgui.ImVec2(mi.x + 8, ma.y - 3),
                imgui.ImVec2(ma.x - 8, ma.y),
                ImVec4toU32(catCol, 0.8), 2)
        end
        
        if imgui.IsItemHovered() then anim(tabId .. "_h", 1, 0.15) else anim(tabId .. "_h", 0, 0.08) end
        imgui.PopStyleVar(2); imgui.PopStyleColor(4)
        
        if ci < row1[#row1] then imgui.SameLine(0, tabGap) end
    end
    ...
    for _, ci in ipairs(row2) do
        ...
        if imgui.Button(u8(cs[ci + 1]) .. "##catr" .. ci, imgui.ImVec2(row2W, tabH)) then
            U.filterCategory[0] = ci
        end
        ...
        if ci < row2[#row2] then imgui.SameLine(0, tabGap) end
    end
end
    imgui.Spacing()
    ...
    if #filt == 0 then
        ...
        for fi, idx in ipairs(filt) do
            ...
            local displayName = (en and "" or "~ ") .. nm
    ...
    if idx ~= S.selectedBindIndex then
        if requestBindAction("select", idx) then
            ...
            btnDl:AddText(imgui.ImVec2(btnMin.x + 12, btnMin.y + 6),
                ImVec4toU32(nameColor), u8(displayName))

            -- Вторая строка: мета-информация
            local metaParts = {}
            if b.cmd and b.cmd ~= "" then
                table.insert(metaParts, "/" .. (b.cmd:match("^(%S+)") or b.cmd))
            end
            if b.key and b.key ~= 0 then
                table.insert(metaParts, getKeyName(b.key, b.key_mod or 0))
            end
            table.insert(metaParts, #(b.steps or {}) .. " шагов")
            if (b.uses or 0) > 0 then
                table.insert(metaParts, b.uses .. " исп.")
            end
            local metaText = u8(table.concat(metaParts, " · "))
    ...
    if not S.selectedBindIndex then
        ...
        -- Большая иконка
        local cx = emptyPos.x + emptyW / 2
        local cy = emptyPos.y + 80
        drawGlow(emptyDl, cx, cy, 30, t.accent, 0.08)
        emptyDl:AddCircle(imgui.ImVec2(cx, cy), 35,
            ImVec4toU32(imgui.ImVec4(t.accent.x, t.accent.y, t.accent.z, 0.15)), 24, 2)
        local plusSize = imgui.CalcTextSize(u8("+"))
        emptyDl:AddText(imgui.ImVec2(cx - plusSize.x / 2, cy - plusSize.y / 2),
            ImVec4toU32(imgui.ImVec4(t.accent.x, t.accent.y, t.accent.z, 0.3)), u8("+"))
    ...
    toggleSwitch("Включён", B.editEnabledBuf)
        -- 6.6: Индикатор несохранённых изменений
    if S.hasUnsavedChanges then
        imgui.SameLine(0, 16)
        local unsavedPulse = (math.sin(os.clock() * 4) + 1) * 0.5
        local unsavedDl = imgui.GetWindowDrawList()
        local dotSp = imgui.GetCursorScreenPos()
        local dotCy = dotSp.y + imgui.GetTextLineHeight() / 2 + 3
        unsavedDl:AddCircleFilled(
            imgui.ImVec2(dotSp.x, dotCy), 4,
            ImVec4toU32(imgui.ImVec4(t.yellow.x, t.yellow.y, t.yellow.z, 0.6 + unsavedPulse * 0.4)), 12)
        drawGlow(unsavedDl, dotSp.x, dotCy, 4, t.yellow, 0.15 + unsavedPulse * 0.15)
        imgui.Dummy(imgui.ImVec2(12, 0))
        imgui.SameLine(0, 4)
        imgui.PushStyleColor(imgui.Col.Text, imgui.ImVec4(t.yellow.x, t.yellow.y, t.yellow.z, 0.7 + unsavedPulse * 0.3))
        imgui.SetCursorPosY(imgui.GetCursorPosY() + 3)
        imgui.Text(u8("Не сохранено"))
        imgui.PopStyleColor()
    end
    -- Бейджи
    imgui.SameLine(0, 16)
    ...
    if S.selectedBindIndex and binds[S.selectedBindIndex] then
        ...
        if selBind.key and selBind.key ~= 0 then
            imgui.SameLine(0, 8)
            badge(getKeyName(selBind.key, selBind.key_mod or 0), t.yellow)
        end
            ...
            if monitorOpen > 0.01 then
                ...
                for _, la in ipairs(lineAnalysis) do
                    if la.isLong or la.isWarning then
                        ...
                        imgui.Text(u8(safeFormat("%d/144", la.processedLen)))
                        imgui.PopStyleColor()
...
if U.fontMono then imgui.PopFont() end
    ...
    if btn(" Сохранить ", 0, 34, "accent") then
        ...
        if name ~= "" and stR ~= "" then
            ...
            saveBinds(); addNotification("Сохранено!", C.NOTIFY_SUCCESS)
            markEditorClean()
            dashData.cachedBindStats = nil
        else
            addNotification("Заполните название и шаги!", C.NOTIFY_ERROR)
        end
    end
...
local function drawSettings()
    ...
    local dd = imgui.new.int(settings.defaultDelay)
    imgui.PushItemWidth(200); imgui.PushStyleVarFloat(imgui.StyleVar.FrameRounding, 14)
    if imgui.InputInt("##sdd", dd, 100, 500) then settings.defaultDelay = clamp(dd[0], C.MIN_DELAY, C.MAX_DELAY); changed = true end
    ...
    local md = imgui.new.int(settings.maxBindDuration)
    imgui.PushItemWidth(200); imgui.PushStyleVarFloat(imgui.StyleVar.FrameRounding, 14)
    if imgui.InputInt("##smd", md, 5000, 10000) then settings.maxBindDuration = clamp(md[0], C.MIN_BIND_DURATION, C.MAX_BIND_DURATION); changed = true end
    imgui.PopStyleVar(); imgui.PopItemWidth(); cardEnd(); imgui.Spacing()
    secTitle("Функции")
    cardBegin("##sfunc", imgui.ImVec2(-1, 400))
    ...
    local ab = imgui.new.bool(settings.autoBackup); if toggleSwitch("Автобэкап каждые 5 мин", ab) then settings.autoBackup = ab[0]; changed = true end
    cardEnd(); imgui.Spacing()
    secTitle("Управление")
    cardBegin("##skeys", imgui.ImVec2(-1, 55), false, 4)
    imgui.PushStyleColor(imgui.Col.Text, t.text2); imgui.AlignTextToFramePadding(); imgui.Text(u8("Стоп-клавиша:")); imgui.PopStyleColor()
    imgui.SameLine(0, 14)
    if btn(" " .. getKeyName(settings.stopKey, settings.stopKeyMod) .. " ##sk", 0, 0, "outline") then
        S.tempKey = settings.stopKey; S.tempMod = settings.stopKeyMod; S.waitingForKey = true; S.keyBindContext = "stop"; S.keyBindPopupActive = true
    end
    cardEnd(); imgui.Spacing()
    secTitle("Оверлей")
    cardBegin("##sovl", imgui.ImVec2(-1, 55), false, 4)
    imgui.PushStyleColor(imgui.Col.Text, t.text2); imgui.AlignTextToFramePadding(); imgui.Text(u8("Позиция:")); imgui.PopStyleColor()
    imgui.SameLine(0, 14)
    if btn("Сверху##op0", 80, 0, settings.overlayPosition == 0 and "accent" or "ghost") then settings.overlayPosition = 0; changed = true end
    imgui.SameLine(0, 8)
    if btn("Снизу##op1", 80, 0, settings.overlayPosition == 1 and "accent" or "ghost") then settings.overlayPosition = 1; changed = true end
    cardEnd(); imgui.Spacing()
    secTitle("AutoFind")
    cardBegin("##safind", imgui.ImVec2(-1, 150))
    imgui.PushStyleColor(imgui.Col.Text, t.text2); imgui.Text(u8("Интервал поиска (мс)")); imgui.PopStyleColor()
    local afi = imgui.new.int(autoFind.interval)
    imgui.PushItemWidth(200); imgui.PushStyleVarFloat(imgui.StyleVar.FrameRounding, 14)
    if imgui.InputInt("##afint", afi, 500, 1000) then autoFind.interval = clamp(afi[0], 1000, 30000) end
    imgui.PopStyleVar(); imgui.PopItemWidth(); imgui.Spacing()
    ...
    local smo = imgui.new.bool(settings.showMembersOverlay); if toggleSwitch("Показывать оверлей", smo) then settings.showMembersOverlay = smo[0]; changed = true end; imgui.Spacing()
    ...
    local mari = imgui.new.int(settings.membersAutoRefreshInterval)
    imgui.PushItemWidth(200); imgui.PushStyleVarFloat(imgui.StyleVar.FrameRounding, 14)
    if imgui.InputInt("##marint", mari, 5000, 10000) then settings.membersAutoRefreshInterval = clamp(mari[0], 10000, 600000); changed = true end
    imgui.PopStyleVar(); imgui.PopItemWidth()
    ...
    cardEnd(); imgui.Spacing()
    secTitle("Оформление")
    local tw = math.floor((imgui.GetContentRegionAvail().x - (#themeOrder-1)*8) / #themeOrder)
    for i, key in ipairs(themeOrder) do
        local th = colorThemes[key]; local act = (settings.currentTheme == key)
        local tid = "theme_"..key
        imgui.PushStyleColor(imgui.Col.ChildBg, th.card)
        imgui.PushStyleVarFloat(imgui.StyleVar.ChildRounding, 16)
        imgui.PushStyleVarFloat(imgui.StyleVar.ChildBorderSize, 0)
        local prevPadT = pushPadding(10, 8)
        imgui.BeginChild("##tc_"..key, imgui.ImVec2(tw, 72), true)
        imgui.GetStyle().WindowPadding = prevPadT
        local isH = imgui.IsWindowHovered(imgui.HoveredFlags.ChildWindows)
        anim(tid.."_h", isH and 1 or 0, 0.12)
        local dl = imgui.GetWindowDrawList(); local wp2 = imgui.GetWindowPos(); local ws2 = imgui.GetWindowSize()
        dl:AddRectFilled(imgui.ImVec2(wp2.x,wp2.y), imgui.ImVec2(wp2.x+ws2.x,wp2.y+ws2.y), ImVec4toU32(th.bg), 16)
        drawGradRounded(dl, wp2.x, wp2.y, ws2.x, 3, th.grad1, th.grad2, 16)
        local dotY = wp2.y + 22; local colors = {th.accent, th.green, th.yellow, th.red}
        for j, c in ipairs(colors) do dl:AddCircleFilled(imgui.ImVec2(wp2.x + 12 + (j-1)*12, dotY), 3.5, ImVec4toU32(c), 12) end
        dl:AddText(imgui.ImVec2(wp2.x+8, wp2.y+40), ImVec4toU32(th.text, 0.8), u8(th.name))
        if act then dl:AddRect(imgui.ImVec2(wp2.x,wp2.y), imgui.ImVec2(wp2.x+ws2.x,wp2.y+ws2.y), ImVec4toU32(th.accent, 0.75), 16, 15, 2)
            drawGlowRect(dl, wp2.x, wp2.y, wp2.x+ws2.x, wp2.y+ws2.y, th.accent, 0.10, 16) end
        imgui.SetCursorPos(imgui.ImVec2(0,0)); imgui.InvisibleButton("##thb_"..key, imgui.ImVec2(tw, 72))
        ...
        if i < #themeOrder then imgui.SameLine(0,8) end
    end
    imgui.Spacing(); thinSep()
    secTitle("Данные")
...
local function drawJournal()
    ...
    for i, f in ipairs(filters) do
        ...
        if btn(f.label .. "##jf_" .. f.id, 0, 26, isActive and "accent" or "ghost") then
            S.journalFilter = f.id
        end
        if i < #filters then imgui.SameLine(0, 6) end
    end
...
local function drawNotes()
    ...
    for i, qi in ipairs(quickInserts) do
        if btn(qi.label .. "##qi" .. i, 0, 24, "ghost") then
            ...
            ffi.copy(B.notesBuf, current .. qi.text)
        end
...
imgui.OnFrame(
    ...
    function(self)
        ...
        setWindowPadding(0,0)
        imgui.SetNextWindowPos(imgui.ImVec2(sizeX/2,sizeY/2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5,0.5))
        imgui.SetNextWindowSize(imgui.ImVec2(1100, 720), imgui.Cond.FirstUseEver)
        imgui.SetNextWindowSizeConstraints(imgui.ImVec2(720, 460), imgui.ImVec2(1400, 920))
        ...
        if logoTexture ~= nil then
            dl:AddImage(logoTexture, imgui.ImVec2(logoX1, logoY1), imgui.ImVec2(logoX2, logoY2), imgui.ImVec2(0, 0), imgui.ImVec2(1, 1), 0xFFFFFFFF)
        else
        ...
        imgui.SetCursorScreenPos(imgui.ImVec2(wp.x + ws.x - 40, wp.y + 8))
        if imgui.InvisibleButton("##close_btn", imgui.ImVec2(28, 28)) then mainWindow.switch2(); mainWindowOpen[0] = false end
        if imgui.IsItemHovered() then anim("close_h", 1, 0.15) else anim("close_h", 0, 0.10) end
        dl:AddText(imgui.ImVec2(wp.x + ws.x - 32, wp.y + 14), ImVec4toU32(lerpColor(t.text3, t.red, closeHov)), u8("x"))
...
if U.fontMono then imgui.PopFont() end
            ...
            if imgui.IsItemHovered() then imgui.BeginTooltip(); imgui.PushStyleColor(imgui.Col.Text, t.green)
                imgui.Text(u8("= " .. (resolveVariable(it.n) or "N/A"))); imgui.PopStyleColor(); imgui.EndTooltip() end
        end
        cardEnd(); imgui.Spacing()
...
local ActionSystem = {
    rpSteps = {
        ...
        dismiss = { "/do КПК находится на поясном держателе.", "/me берёт в руки свой КПК и включает его, авторизуется в системе ЕВДО.", "/me находит в системе профиль нужного гос.служащего, вносит в него корректировки.", "/do Изменения успешно сохранены - сотрудник уволен по решению агентов ФБР." },
        wanted = {},
    },
    buildCommand = {
...
local function sendActionRP(actionType, pid, reason, extra)
    ...
    local pname = (function() if sampIsPlayerConnected(pid) then return sampGetPlayerNickname(pid) or ("ID_"..pid) end; return "ID_"..pid end)()
    ...
    if steps then for _, s2 in ipairs(steps) do if S.stopCurrentBind then break end; safeSendChat(s2); wait(settings.defaultDelay) end end
    if not S.stopCurrentBind then
        local cmdBuilder = ActionSystem.buildCommand[actionType]
        if cmdBuilder then local msg = cmdBuilder(pid, reason, extra); safeSendChat(msg)
            local nb = ActionSystem.notifyText[actionType]; if nb then addNotification(nb(reason, extra), C.NOTIFY_SUCCESS) end end
        if extra.postSteps then for _, s2 in ipairs(extra.postSteps) do if S.stopCurrentBind then break end; safeSendChat(s2); wait(settings.defaultDelay) end end
    end
...
local demouteTabIdx = imgui.new.int(0)
local DismissMenuWindow = imgui.new.bool(false)
local dismissTabIdx = imgui.new.int(0)
...
-- Универсальный рендер меню выбора причины
local function renderReasonMenu(windowBool, title, dataList, namesList, onSelect)
    if #dataList == 0 then imgui.TextColored(imgui.ImVec4(1,0.2,0.2,1), u8("Данные не найдены!"))
    else
        for i, name in ipairs(namesList) do
            if imgui.CollapsingHeader(u8(name) .. "##sec" .. title .. i) then
                imgui.Indent(16); local sec = dataList[i]
                if sec and sec.item then for j, item in ipairs(sec.item) do
                    local txt = item.reason or ("Пункт " .. j)
                    if item.text then txt = txt .. " — " .. item.text:sub(1,100) end
                    if item.lvl then txt = txt .. " [" .. item.lvl .. "]" end
                    if btn(txt .. "##" .. title .. i .. "_" .. j, -1, 26, "default") then
                        windowBool[0] = false; onSelect(item)
                    end; imgui.Spacing()
                end end; imgui.Unindent(16)
            end
        end
    end
end
...
imgui.OnFrame(function() return SumMenuWindow[0] end, function(self)
 local t = T()
    imgui.SetNextWindowPos(imgui.ImVec2(sizeX/2,sizeY/2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5,0.5))
    imgui.SetNextWindowSize(imgui.ImVec2(800, 550), imgui.Cond.FirstUseEver)
    pushWindowPadding(16,12)
    imgui.Begin(u8(" Выговор"), SumMenuWindow, imgui.WindowFlags.NoCollapse)
    secTitle("Выговор [" .. S.selectedPlayerId .. "] " .. S.selectedPlayerName)
    renderReasonMenu(SumMenuWindow, "gw", ustav_data, ustav_names, function(item)
...
local t = T()
    ...
    secTitle("Розыск [" .. S.selectedWantedPlayerId .. "] " .. S.selectedWantedPlayerName)
...
local t = T()
    ...
    secTitle("Demoute [" .. S.selectedDemoutePlayerId .. "] " .. S.selectedDemoutePlayerName)
    imgui.Spacing(); imgui.PushStyleColor(imgui.Col.Text, t.text2); imgui.Text(u8("Дней запрета(0-14):")); imgui.PopStyleColor()
    ...
    if btn("УК##demTab0", 140, 32, demouteTabIdx[0] == 0 and "accent" or "ghost") then demouteTabIdx[0] = 0 end
    imgui.SameLine(0, 8)
    if btn("Уставы##demTab1", 140, 32, demouteTabIdx[0] == 1 and "accent" or "ghost") then demouteTabIdx[0] = 1 end
    imgui.Spacing(); thinSep(); imgui.Spacing()
    imgui.BeginChild("##demouteScroll", imgui.ImVec2(0, -50), false)
    local demData = demouteTabIdx[0] == 0 and uk_data or ustav_data
    local demNames = demouteTabIdx[0] == 0 and uk_names or ustav_names
...
local t = T()
    ...
    secTitle("Dismiss [" .. S.selectedDismissPlayerId .. "] " .. S.selectedDismissPlayerName)
    imgui.Spacing()
    if btn("УК##disTab0", 140, 32, dismissTabIdx[0] == 0 and "accent" or "ghost") then dismissTabIdx[0] = 0 end
    imgui.SameLine(0, 8)
    if btn("Уставы##disTab1", 140, 32, dismissTabIdx[0] == 1 and "accent" or "ghost") then dismissTabIdx[0] = 1 end
    imgui.Spacing(); thinSep(); imgui.Spacing()
    imgui.BeginChild("##dismissScroll", imgui.ImVec2(0, -50), false)
    local disData = dismissTabIdx[0] == 0 and uk_data or ustav_data
    local disNames = dismissTabIdx[0] == 0 and uk_names or ustav_names
...
imgui.OnInitialize(function()
    ...
    local cyrillic = io.Fonts:GetGlyphRangesCyrillic()
    ...
    local function loadFont(primaryPath, fallbackPath, size)
        if doesFileExist(primaryPath) then
            return io.Fonts:AddFontFromFileTTF(primaryPath, size, nil, cyrillic)
        elseif doesFileExist(fallbackPath) then
            return io.Fonts:AddFontFromFileTTF(fallbackPath, size, nil, cyrillic)
        end
        return nil
    end
    ...
    if doesFileExist(logoPath) then
        ...
        if success and texture ~= nil then logoTexture = texture end
    end
    ...
    if doesFileExist(avatarPath) then
        ...
        if success and texture ~= nil then avatarTexture = texture end
    end
end)
...
function sampev.onServerMessage(color, text)
    local ok, result = pcall(function()
        ...
        if autoFind.active and autoFind.lastId ~= -1 then
            if string.find(text, "Местоположение (%w+_%w+)%[(%d+)%] отмечено на карте") then
                ...
                if nick ~= autoFind.lastNick then
    ...
    if not ok then print("[SH] onServerMessage error: " .. tostring(result)) end
    return result
end
...
function sampev.onShowTextDraw(id, data)
    ...
    if not ok then print("[SH] onShowTextDraw error: " .. tostring(result)) end
    return result
end
...
function sampev.onTextDrawSetString(id, text)
    ...
    if not ok then print("[SH] onTextDrawSetString error: " .. tostring(result)) end
    return result
end
...
function sampev.onShowDialog(dialogid, style, title, button1, button2, text)
    local ok, result = pcall(function()
        ...
        if #membersOverlay.tempList > 0 then
            ...
            membersOverlay.lastUpdate = os.clock(); membersOverlay.isChecking = false; S.membersOverlayVisible = true
        else membersOverlay.tempList = {}; membersOverlay.isChecking = false; addNotification("Members: список пуст!", C.NOTIFY_WARNING) end
        sampSendDialogResponse(dialogid, 0, 0, 0); return false
    end)
    if not ok then print("[SH] onShowDialog error: " .. tostring(result)); membersOverlay.isChecking = false end
    return result
end
...
-- =========================================================================
imgui.OnFrame(
    function() return settings.showMembersOverlay end,
    function(self)
        local t = T(); local now = os.clock()
        ...
        -- === СБОР ТЕКСТОВ И РАСЧЁТ МАКСИМАЛЬНОЙ ШИРИНЫ ===
        local texts = {}
        local maxTextW = 0

        -- Временно устанавливаем полужирный шрифт для точного расчёта ширины
        if U.fontSemibold then imgui.PushFont(U.fontSemibold) end

        local headerStr = u8("Сотрудников онлайн: " .. (hasData and #membersOverlay.list or 0))
        local headerW = imgui.CalcTextSize(headerStr).x
        if headerW > maxTextW then maxTextW = headerW end
        table.insert(texts, { str = headerStr, w = headerW, col = imgui.ImVec4(1, 1, 1, 0.9), isHeader = true })

        if hasData then
            for i = 1, listCount do
                local m = membersOverlay.list[i]; local parts = {}
                if m.afk > 0 then table.insert(parts, safeFormat("AFK:%d", m.afk)) end
                if m.warns and m.warns > 0 then table.insert(parts, "W:" .. m.warns) end
                if m.info and m.info ~= "" and m.info ~= "-" then table.insert(parts, m.info) end
                table.insert(parts, m.nick)
                table.insert(parts, "(" .. m.id .. ")")
                table.insert(parts, "[" .. m.rank_number .. "]")
                local str = u8(table.concat(parts, " "))
                local w = imgui.CalcTextSize(str).x
                if w > maxTextW then maxTextW = w end
                local col = m.working
                    and imgui.ImVec4(t.green.x, t.green.y, t.green.z, 0.85)
                    or imgui.ImVec4(t.red.x, t.red.y, t.red.z, 0.65)
                table.insert(texts, { str = str, w = w, col = col })
            end
            if #membersOverlay.list > maxVisible then
                local str = u8("... +" .. (#membersOverlay.list - maxVisible))
                local w = imgui.CalcTextSize(str).x
                if w > maxTextW then maxTextW = w end
                table.insert(texts, { str = str, w = w, col = imgui.ImVec4(t.text3.x, t.text3.y, t.text3.z, 0.35) })
            end
        else
            ...
            local w = imgui.CalcTextSize(str).x
            if w > maxTextW then maxTextW = w end
            local col = membersOverlay.isChecking
                and imgui.ImVec4(t.yellow.x, t.yellow.y, t.yellow.z, 0.7)
                or imgui.ImVec4(t.text3.x, t.text3.y, t.text3.z, 0.5)
            table.insert(texts, { str = str, w = w, col = col })
        end

        -- Снимаем шрифт после расчётов
        if U.fontSemibold then imgui.PopFont() end

        -- Размер окна: ширина текста + запас
        local overlayW = maxTextW + 60
        ...
        -- Устанавливаем полужирный шрифт для отрисовки текста
        if U.fontSemibold then imgui.PushFont(U.fontSemibold) end

        -- Dummy задаёт ширину окна
        imgui.Dummy(imgui.ImVec2(overlayW, 0))

        -- Рисуем текст через DrawList
        local dl = imgui.GetWindowDrawList()
        local wp = imgui.GetWindowPos()
        local curY = wp.y + 4

        for i, entry in ipairs(texts) do
            local textX = wp.x + overlayW - entry.w - 6   -- выравнивание по правому краю
            dl:AddText(imgui.ImVec2(textX, curY), ImVec4toU32(entry.col), entry.str)
            dl:AddText(imgui.ImVec2(textX + 1, curY), ImVec4toU32(entry.col, 0.5), entry.str) -- эффект "жирности"
        ...
        if cursorActive then
            ...
            if math.abs(newRX - settings.membersOverlayRightX) > 2 or math.abs(newPY - settings.membersOverlayPosY) > 2 then
                ...
                if not membersOverlay._savePending then
                    membersOverlay._savePending = true
                    lua_thread.create(function() wait(1000); saveSettings(); membersOverlay._savePending = false end)
                end
            end
        end
...
function main()
    ...
    initVariables(); registerBindCommands(); loadActivityLog()
    ...
    lua_thread.create(function()
        wait(5000); local retries = 0
        while settings.showMembersOverlay and #membersOverlay.list == 0 and not membersOverlay.isChecking and retries < 3 do
            membersOverlay.isChecking = true;membersOverlay._checkStartTime = os.clock(); membersOverlay.tempList = {}; safeSendChat("/members"); retries = retries + 1; wait(10000)
        end
        while true do wait(1000)
            if settings.membersAutoRefresh and settings.showMembersOverlay and not membersOverlay.isChecking and membersOverlay.lastUpdate > 0 then
                if (os.clock() - membersOverlay.lastUpdate) * 1000 >= settings.membersAutoRefreshInterval then
                    membersOverlay.isChecking = true; membersOverlay._checkStartTime = os.clock(); membersOverlay.tempList = {}; safeSendChat("/members") end
            end
        end
    end)
    ...
    sampAddChatMessage("{ff0000}[Snatch Helper] {ff6666}v" .. C.SCRIPT_VERSION .. " | F3 — меню | /snatch", -1)
    ...
    addEventHandler('onWindowMessage', function(msg, wparam)
        if msg == 0x100 or msg == 0x104 then
            if S.waitingForKey and S.keyBindPopupActive then
                if wparam == 0x1B then S.waitingForKey = false
                elseif wparam ~= 0x10 and wparam ~= 0x11 and wparam ~= 0x12 then
                    S.tempKey = wparam; S.tempMod = 0
                    if isKeyDown(0x11) then S.tempMod = 1 elseif isKeyDown(0x10) then S.tempMod = 2 elseif isKeyDown(0x12) then S.tempMod = 3 end
    ...
    while true do
        ...
        if os.clock() - S.animCleanupTimer > C.ANIM_CLEANUP_INTERVAL then S.animCleanupTimer = os.clock(); cleanupAnims() end
        -- Автобэкап + УЛУЧШЕНИЕ #19
        if settings.autoBackup and os.clock() - S.lastAutoBackup > C.AUTO_BACKUP_INTERVAL then S.lastAutoBackup = os.clock(); autoBackup() end
        -- Стоп-клавиша
        if S.activeBinder or ActionSystem.activeFlags.gwarn or ActionSystem.activeFlags.wanted or ActionSystem.activeFlags.demoute or ActionSystem.activeFlags.dismiss then
            if settings.stopKey ~= 0 and checkKeyCombo(settings.stopKey, settings.stopKeyMod) then S.stopCurrentBind = true; addNotification("Стоп...", C.NOTIFY_WARNING) end
        end
        -- Хоткеи биндов
        if not S.activeBinder and not mainWindow.state and not S.keyBindPopupActive then
            for _, b in ipairs(binds) do
                if b.enabled ~= false and b.key and b.key ~= 0 and checkKeyCombo(b.key, b.key_mod or 0) then
                    local canRun = true
                    if b.cooldown and b.cooldown > 0 and b.lastUseTime then if (os.clock() - b.lastUseTime)*1000 < b.cooldown then canRun = false end end
                    if canRun then local ref = b; lua_thread.create(function() performBind(ref, {}) end) end
                    break
                end
            end
        end
...
function onScriptTerminate(scr, quitGame)
    if scr == thisScript() then
        saveSettings(); saveBinds(); saveActivityLog(); unregisterBindCommands()
